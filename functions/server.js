...
router.get("/", function (req, res) {
    ...
    <head>
        ...
        <title>Microservice Utility</title>
        <link href="https://i.ibb.co/4pshfgd/settings-gear-favicon.png" rel=icon type=image/png>
        <style>
            body {
            ...
            footer {
                ...
                background-color: #fafafa;
                box-shadow: 0 -1px 8px -1px rgba(128, 128, 128, 0.5);
            }
            ...
            #url_input:focus {
                ...
                border-color: black;
                box-shadow: 0 0 5px 3px rgba(128, 128, 128, 0.2);
            }
        ...
        <script>
            function updateHTML(selection) {
                const parentElement = document.getElementsByClassName('container');
                const childElement = document.getElementsByClassName('content');
                ...
                switch (selection) {
                    case 'timestamp':
                        createElement.innerHTML = '<h2>Timestamp API</h2><h3>Example Usage:</h3><ul><li><a href="api/2022-01-15" title="Click to view the API in action">[project url]/api/2022-01-15</a><li><a href="api/1451001600000" title="Click to view the API in action">[project url]/api/1642204800000</a></ul><h3 class="exampleoutput">Example Output:</h3><p><code>{"unix":1642204800000,"utc":"Sat, 15 Jan 2022 00:00:00 GMT"}</code></p>';
                        break;
                    case 'whoami':
                        createElement.innerHTML = '<h2>Who Am I API</h2><h3>Example Usage:</h3><ul><li><a href="api/whoami" title="Click to view the API in action">[project url]/api/whoami</a></ul><h3 class="exampleoutput">Example Output:</h3><p><code>{"ipaddress":"127.0.0.1","language":"en-US,en;q=0.5","software":"Mozilla/5.0 (X11;Ubuntu;Linux x86_64;rv:95.0) Gecko/20100101 Firefox/95.0"}</code></p>';
                        break;
                    case 'urlshortener':
                        createElement.innerHTML = '<h2>URL Shortener API</h2> <h3>Example Usage:</h3> <ul> <li><a href="api/shorturl/1" title="Click to view the API in action">[project url]/api/shorturl/1</a> </ul> <section> <form action="api/shorturl" method="POST"> <fieldset> <legend align="center">URL Shortener</legend> <label id="url_label" for="url_input">URL:</label> <input id="url_input" type="text" name="url" placeholder="https://www.freecodecamp.org/" title="Enter URL" required> <input class="btn" id="submit-btn" type="submit" value="POST URL" title="Submit to shorten URL"> </fieldset> </form> </section> <h3 class="exampleoutput">Example Output:</h3> <p><code>{"original_url":"https://www.freecodecamp.org/","short_url":1}</code></p>';
                        break;
                    case 'metadata':
                        createElement.innerHTML = '<h2>File Metadata Shortener API</h2> <h3>Example Usage:</h3> <p class="upload-file-msg">Please Upload a File...</p> <section> <form enctype="multipart/form-data" action="api/filemetadata" method="POST"> <fieldset> <legend align="center">File Analyzer</legend> <input class="upfile" id="inputfield" type="file" name="upfile" title="Choose a file to upload" required> <input class="btn upload-btn" id="submit-btn" type="submit" value="Upload&#x21A5;" title="Submit to analyze your file"> </fieldset> </form> </section> <h3 class="exampleoutput">Example Output:</h3> <p><code>{"name":"github_icon.png", "type":"image/png","size":224}</code></p>';
                        break;
                    default:
                        createElement.innerHTML = '<h3>Click any of the Microservice API Button to view their usage and examples</h3>';
                        break;
                }
                parentElement[0].replaceChild(createElement, childElement[0]);
            }
        </script>
    </head>
    
    <body>
        <h2>Microservice Utility </h2>
        <hr>
        <div class="row">
            <button class="btn" onclick="updateHTML('timestamp')" name="timestamp"
                title="Click for more info about Timestamp Microservice API">Timestamp Microservice</button>
            <button class="btn" onclick="updateHTML('whoami')" name="whoami"
                title="Click for more info about Who Am I Microservice API">Who Am I Microservice</button>
            <button class="btn" onclick="updateHTML('urlshortener')" name="urlshortener"
                title="Click for more info about URL Shortener Microservice API">URL Shortener Microservice</button>
            <button class="btn" onclick="updateHTML('metadata')" name="metadata"
                title="Click for more info about File Metadata Microservice API">File Metadata Microservice</button>
        </div>
        <div class="container">
            <div class="content">
                <h3>A set of different Microservice APIs built using Express JS which serve different purpose. </h3>
                <p class="description">Click the microservice API buttons above to find more info about the APIs and how to
                    use them.</p>
            </div>
        </div>
        <footer>
            <p class="footer">By <a href="https://github.com/saideepd" title="View Saideep's GitHub projects">Saideep
                    Dicholkar </a>
                <img alt="GitHub Logo" src="https://i.ibb.co/W0Jhcgj/github-icon-grey.png">
            </p>
        </footer>
    </body>
